...
var ccm_parseJSON = function (resp, onNoError) {
	if (resp.error) {
		alert(resp.message);	
	} else {
		onNoError();
...
var ccm_deactivateSite = function (onDone) {
...
var ccm_addHeaderItem = function (item, type) {
        var i, ss;
	var doLoad = true;
	if (type === 'CSS') {
